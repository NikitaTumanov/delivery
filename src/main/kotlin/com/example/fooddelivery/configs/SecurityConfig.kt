...
import com.example.fooddelivery.JwtFilter
...
class SecurityConfig(
    ...
    //    configuration.allowedMethods = Arrays.asList("GET","POST","UPDATE","OPTIONS","DELETE")
    ...
    override fun configure(http: HttpSecurity?) {
        http!!.httpBasic().disable().cors()
            ...
            .antMatchers("/registration","/login","/add_user","/public/**").not().authenticated()
            //Доступ только для пользователей с ролью Администратор
            .antMatchers("/admin/**").hasRole("ADMIN")
            //Доступ только для пользователей с ролью Курьер
